...
Runtime client (execute mode)
-----------------------------

For systems that need to call Terraform operations from Python (for example touchscale-vm execute mode), terraformpy also exposes a minimal runtime client.

Constructors exported at module root:

* ``Client(...)``
* ``TerraformPy(...)`` (alias of ``Client``)

Supported constructor kwargs:

* ``working_dir="..."``
* ``chdir="..."``
* ``cwd="..."``
* no kwargs are also supported
...
* ``destroy(...)``
* ``output(...)``
* ``init(...)`` (optional helper)

Return contract:

* ``plan/apply/destroy/init`` return an integer return code (`0` on success).
* ``output`` returns a Terraform output map as a Python ``dict`` on success, or non-zero integer return code on failure.

.. code-block:: python

    from terraformpy import Client, TerraformPy

    # any of these constructors are valid
    client = Client()
